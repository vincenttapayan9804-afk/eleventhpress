  ...
  return (
    <div className="page-enter mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <button onClick={() => setView("media")} className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> {t("backToNewsNotes")}
      </button>
      ...
      <p className="mt-6 border-t border-foreground/10 pt-4 text-sm font-medium">{t("byAuthorDate", { author: post.authorName, date: postDate(post.publishedAt) })}</p>
      {post.heroImageUrl && (
