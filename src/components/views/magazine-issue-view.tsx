...
function PieceReader({ piece, onBack }: { piece: Piece; onBack: () => void }) {
  return (
    <div className="page-enter mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <p className="mt-6 border-t border-foreground/10 pt-4 text-sm font-medium">By {authorNames(piece.authors)}</p>
      {piece.heroImageUrl && (
...
export function MagazineIssueView() {
  ...
  return (
    <div className="page-enter mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="flex gap-2">
          {issue.pdfUrl && (
            <Button asChild variant="outline" size="sm">
              <a href={issue.pdfUrl} target="_blank" rel="noopener noreferrer"><FileText className="mr-1.5 h-3.5 w-3.5" /> PDF</a>
            </Button>
          )}
          {issue.epubUrl && (
            <Button asChild variant="outline" size="sm">
              <a href={issue.epubUrl} target="_blank" rel="noopener noreferrer"><Download className="mr-1.5 h-3.5 w-3.5" /> EPUB</a>
            </Button>
          )}
        </div>
      </div>
      ...
      {coverStory && (
        ...
        >
          ...
          <div className="flex flex-col justify-center px-8 py-10 sm:px-12 sm:py-14">
            <p className="font-mono text-[0.7rem] uppercase tracking-[0.22em] text-[oklch(0.76_0.11_294)]">{coverStory.category} · Cover story</p>
            ...
            <p className="mt-6 text-sm font-medium text-white/60">By {authorNames(coverStory.authors)}</p>
          </div>
        </button>
      )}
      ...
      {rest.length > 0 && (
        <div className="mt-14">
          <p className="eyebrow border-b border-foreground/10 pb-3">In this issue</p>
          <div className="mt-6 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {rest.map((piece, i) => (
              ...
              >
                ...
                <p className="mt-3 text-xs font-medium text-muted-foreground">By {authorNames(piece.authors)}</p>
              </button>
            ))}
          </div>
        </div>
