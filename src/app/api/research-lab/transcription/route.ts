...
export async function POST(req: NextRequest) {
  ...
  const quota = await checkResearcherQuota(session.userId, RESEARCH_MODULE_KEYS.TRANSCRIPTION);
  if (!quota.allowed) {
    return NextResponse.json(
      { error: `Monthly transcription limit reached for your plan (${quota.used}/${quota.limit}). Upgrade or wait for next month.` },
      { status: 429 }
    );
  }
