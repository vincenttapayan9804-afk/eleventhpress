...
import { getPlatform, buildShareKit, PLATFORMS } from "@/lib/distribution";
...
/**
 * POST /api/distribution { articleId, platform }
 * Generates (or regenerates) the share kit for a platform. Upserts on the
 * (articleId, platform) unique constraint so re-clicking "Generate" just
 * refreshes the content rather than creating duplicate rows.
 */
export async function POST(req: NextRequest) {
  const session = getSessionFromHeaders(req.headers);
  if (!session) return NextResponse.json({ error: "Unauthenticated" }, { status: 401 });

  const { articleId, platform } = (await req.json()) as { articleId?: string; platform?: string };
