...
import { buildBibTeX, buildRis } from "@/lib/citation-export";

/**
 * GET /api/articles/[id]/export?format=ris|bibtex
 ...
 * copy-to-clipboard-only citation flow. Same access rule as
 * /api/articles/[id]: PUBLISHED is public, anything still in the pipeline
 * requires the corresponding author or editorial staff.
 */
export async function GET(
  ...
  const body = format === "ris" ? buildRis(exportable) : buildBibTeX(exportable);
  const contentType =
